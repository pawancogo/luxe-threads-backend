...
# Shared module for seeding navigation items
# Can be used in rake tasks, migrations, or seeds.rb
module NavigationItemsSeeder
  def self.seed_navigation_items
    navigation_items = [
      {
        key: 'dashboard',
        label: 'Dashboard',
    ...
    if ActiveRecord::Base.connection.table_exists?('navigation_items')
      navigation_items.each do |item_data|
        ...
        if item.save
          puts "✓ #{item.label} (#{item.key}) - #{item.new_record? ? 'Created' : 'Updated'}"
        else
          puts "✗ Error saving #{item_data[:label]}: #{item.errors.full_messages.join(', ')}"
        end
      end
      
      puts "\n✓ Navigation items seeding completed!"
      puts "Total items: #{NavigationItem.count}"
      puts "System items: #{NavigationItem.where(is_system: true).count}"
      true
    else
      puts "✗ Error: navigation_items table does not exist. Please run migrations first."
      false
    end
  end
end
