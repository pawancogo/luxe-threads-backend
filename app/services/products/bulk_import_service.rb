...
module Products
  class BulkImportService < BaseService
    ...
    attr_reader :supplier_profile, :csv_content, :results
    ...
    def valid_csv?
      if @csv_content.blank?
        add_error('CSV content is required')
        return false
      end
      true
    end
    ...
    def parse_csv
      ...
      if rows.empty?
        add_error('CSV file is empty or has no data rows')
        return []
      end
    ...
    def create_product(product_data, row_number)
      service = Products::CreationService.new(@supplier_profile, product_data.except(:errors))
      result = service.call
