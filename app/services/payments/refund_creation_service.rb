# frozen_string_literal: true

# Service for creating payment refunds
module Payments
  class RefundCreationService < BaseService
    attr_reader :payment_refund

    def initialize(payment, refund_params, user)
      super()
      @payment = payment
      @refund_params = refund_params
      @user = user
    end

    def call
      validate_authorization!
      create_refund
      update_payment_status
      set_result(@payment_refund)
      self
    rescue StandardError => e
      handle_error(e)
      self
    end

    private

    def validate_authorization!
      unless @payment.user_id == @user.id || @user.admin?
        add_error('Not authorized to create refund for this payment')
        raise StandardError, 'Unauthorized'
      end
    end

    def create_refund
      @payment_refund = @payment.payment_refunds.build(
        order: @payment.order,
        amount: @refund_params[:amount] || @payment.amount,
        currency: @refund_params[:currency] || @payment.currency,
        reason: @refund_params[:reason] || 'Refund requested',
        description: @refund_params[:description],
        status: 'pending',
        processed_by: @user.admin? ? @user : nil
      )
      
      unless @payment_refund.save
        add_errors(@payment_refund.errors.full_messages)
        raise ActiveRecord::RecordInvalid, @payment_refund
      end
    end

    def update_payment_status
      if @payment_refund.amount >= @payment.amount
        @payment.update!(status: 'refunded')
      else
        @payment.update!(status: 'partially_refunded')
      end
    end
  end
end

